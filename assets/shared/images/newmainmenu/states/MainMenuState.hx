...
class MainMenuState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		// --- BACKGROUNDS (Common to both menus) ---
		...
		positionArrows();
		changeSelection();
		// --- PLAY SELECT MENU ITEMS (Mode 1: Story vs Freeplay) ---
		
		// 1. Blender (Blur Background)
		...
		blender.alpha = 0; // Start transparent for fade-in
		add(blender);
		// 2. Story Mode Button (Top)
		storyBtn = new FlxSprite(0, 0).loadGraphic(Paths.image('newmainmenu/storymode'));
		storyBtn.scale.set(0.7, 0.7); // Adjust scale if needed
